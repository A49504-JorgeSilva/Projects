...
class UI {
    ...
    ///pegar os argumentos do utilizador verificando as restrições (acabar restrictions tmb os booleans teem de vir tdos de la)
    /// antes de chamar a função que faaz o query (a do model)
    /// mesma coisa para todas as em app depois envair resultatos para print resutls(acabar print results)
    private void startStopTravel(){
        Scanner scanner = new Scanner(System.in);
        String[] parameterString = scanner.nextLine().split(" ");
        if(parameterString.length != 4){
            System.out.println("must only insert 4 arguments");
        }
        if(!Objects.equals(parameterString[0], "start") && !Objects.equals(parameterString[0], "stop")){
            System.out.println("must type start or stop as first argument");
        }
        if(parameterString[2].matches("\\d+")){
            System.out.println("station ID contains non digits");
        }
        if(parameterString[3].matches("\\d+")){
            System.out.println("scooter ID contains non digits");
        }
        Model.travel(parameterString);
        scanner.close();
        System.out.println("startStopTravel()");
    }

    ///quando a chamar updateDocks enviar/não enviar parametro scooter no caso de querer inserir/remover scooter da dock
