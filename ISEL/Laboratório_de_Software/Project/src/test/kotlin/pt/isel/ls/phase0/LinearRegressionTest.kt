...
class LinearRegressionTest {
    ...
    @Test
    fun `normalize scale with negative numbers`(){
        val scale = Scale(listOf(-10.0, 0.0, 10.0))
        assertEquals(0.0, scale.normalize(-10.0), 1e-9)
        assertEquals(0.5, scale.normalize(0.0), 1e-9)
        assertEquals(1.0, scale.normalize(10.0), 1e-9)
    }

    @Test
    fun `denormalize should invert normalize with negative numbers`(){
    ...
    @Test
    fun `scale should handle values outside the range`(){
        val scale = Scale(listOf(5.0, 10.0, 15.0))

        val belowMin = scale.normalize(0.0)
        assertTrue(belowMin < 0.0)

        val aboveMax = scale.normalize(20.0)
        assertTrue(aboveMax > 1.0)
    }

    @Test
    fun `denormalize should invert normalize with numbers outside the range`(){
        val scale = Scale(listOf(5.0, 10.0, 15.0))

        val belowMinValue = 0.0
        val belowMin = scale.normalize(belowMinValue)

        val aboveMaxValue = 20.0
        val aboveMax = scale.normalize(aboveMaxValue)

        assertEquals(belowMinValue, scale.denormalize(belowMin), 1e-9)
        assertEquals(aboveMaxValue, scale.denormalize(aboveMax), 1e-9)
    }
