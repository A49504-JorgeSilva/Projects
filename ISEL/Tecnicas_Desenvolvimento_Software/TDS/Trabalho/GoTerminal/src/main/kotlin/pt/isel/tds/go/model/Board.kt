...
fun Board.play(position: Position): PlayInfo = when(this) {
    is BoardRun-> {
        check(moves[position] == null) { "Position $position used" }
        check(position.isValidMove(moves, this.groups, turn) ) { "Position $position is not valid (liberty rule)" }
        ...
        val score = capturedGroups.map { it.positions.size }.sum()
